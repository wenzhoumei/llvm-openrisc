...
SDValue OR1KTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_OR1K);
...
SDValue
OR1KTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_OR1K);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_OR1K);
...
bool OR1KTargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context,
    const Type *RetTy) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, RVLocs, Context);
  return CCInfo.CheckReturn(Outs, RetCC_OR1K);
}
...
OR1KTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  SmallVector<CCValAssign, 16> RetLocs;
  CCState RetCCInfo(CallConv, IsVarArg, MF, RetLocs, *DAG.getContext());
  RetCCInfo.AnalyzeReturn(Outs, RetCC_OR1K);
  ...
  return DAG.getNode(OR1KISD::RET, DL, MVT::Other, RetOps);
}

SDValue OR1KTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
const char *OR1KTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case OR1KISD::BR_JT:
    return "OR1KISD::BR_JT";
  case OR1KISD::CALL:
    return "OR1KISD::CALL";
  case OR1KISD::PCREL_WRAPPER:
    return "OR1KISD::PCREL_WRAPPER";
  case OR1KISD::RET:
    return "OR1KISD::RET";
  case OR1KISD::SELECT_CC:
    return "OR1KISD::SELECT_CC";
  case OR1KISD::GA_WRAPPER:
    return "OR1KISD::GA_WRAPPER";
  }
  return nullptr;
}

SDValue OR1KTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(OR1KISD::GA_WRAPPER, SDLoc(Op), getPointerTy(DL), Result);
}

SDValue OR1KTargetLowering::LowerSelectCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDLoc dl(Op);
  unsigned OR1KCC = (unsigned) CC;

  return DAG.getNode(OR1KISD::SELECT_CC, dl, TrueVal.getValueType(), LHS, RHS, TrueVal, FalseVal,
                     DAG.getConstant(OR1KCC, dl, MVT::i32));
}
...
MachineBasicBlock *
OR1KTargetLowering::expandSelectCC(MachineInstr &MI, MachineBasicBlock *BB) const {
  ...
  switch (CC) {
    case ISD::SETEQ:
      CondSetOpcode = OR1K::SFEQ; // Replace OR1K with your target's namespace
      break;
    case ISD::SETNE:
      CondSetOpcode = OR1K::SFNE;
      break;
    case ISD::SETUGT:
      CondSetOpcode = OR1K::SFGTU;
      break;
    case ISD::SETUGE:
      CondSetOpcode = OR1K::SFGEU;
      break;
    case ISD::SETULT:
      CondSetOpcode = OR1K::SFLTU;
      break;
    case ISD::SETULE:
      CondSetOpcode = OR1K::SFLEU;
      break;
    case ISD::SETGT:
      CondSetOpcode = OR1K::SFGTS;
      break;
    case ISD::SETGE:
      CondSetOpcode = OR1K::SFGES;
      break;
    case ISD::SETLT:
      CondSetOpcode = OR1K::SFLTS;
      break;
    case ISD::SETLE:
      CondSetOpcode = OR1K::SFLES;
      break;
    default:
  ...
  BuildMI(ThisMBB, dl, TII.get(CondSetOpcode))
    .addReg(OR1K::SR_FFlag)
    .addReg(MI.getOperand(1).getReg())
    .addReg(MI.getOperand(2).getReg());

  BuildMI(ThisMBB, dl, TII.get(OR1K::BF))
    .addReg(OR1K::SR_FFlag)
    .addMBB(SinkMBB);
  ...
  BuildMI(*SinkMBB, SinkMBB->begin(), dl, TII.get(OR1K::PHI),
