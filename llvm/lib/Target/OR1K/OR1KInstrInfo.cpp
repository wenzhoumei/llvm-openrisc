//===- OR1KInstrInfo.cpp - OR1K Instruction Information ---------------===//
//
...
OR1KInstrInfo::OR1KInstrInfo(const OR1KSubtarget &STI)
    : OR1KGenInstrInfo(OR1K::ADJCALLSTACKDOWN, OR1K::ADJCALLSTACKUP),
      RI(STI), STI(STI) {}

Register OR1KInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                              int &FrameIndex) const {
  if (MI.getOpcode() == OR1K::LWS) {
...
Register OR1KInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                             int &FrameIndex) const {
...
void OR1KInstrInfo::adjustStackPtr(unsigned SP, int64_t Amount,
  ...
  if (isInt<16>(Amount)) { // addi sp, sp, amount
    BuildMI(MBB, I, DL, get(OR1K::ADDI), Reg).addReg(SP).addImm(Amount);
  } else { // Expand immediate that doesn't fit in 16-bit.
    unsigned Reg1;
    loadImmediate(MBB, I, &Reg1, Amount);
    BuildMI(MBB, I, DL, get(OR1K::ADD), Reg)
        .addReg(SP)
        .addReg(Reg1, RegState::Kill);
  }

  BuildMI(MBB, I, DL, get(OR1K::OR), SP)
...
void OR1KInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (OR1K::GPRRegClass.contains(DestReg, SrcReg))
    BuildMI(MBB, MBBI, DL, get(OR1K::OR), DestReg)
...
void OR1KInstrInfo::storeRegToStackSlot(
...
void OR1KInstrInfo::loadRegFromStackSlot(
...
void OR1KInstrInfo::getLoadStoreOpcodes(const TargetRegisterClass *RC,
                                          unsigned &LoadOpcode,
                                          unsigned &StoreOpcode,
                                          int64_t offset) const {
  assert((RC == &OR1K::GPRRegClass) &&
...
void OR1KInstrInfo::loadImmediate(MachineBasicBlock &MBB,
                                    MachineBasicBlock::iterator MBBI,
  ...
  *Reg = RegInfo.createVirtualRegister(RC);
  if (Value >= -32768 && Value <= 32767) { // if 16 bits
    ...
    BuildMI(MBB, MBBI, DL, get(OR1K::ADDI), *Reg)
        .addReg(OR1K::R0)    // R0 as the source register
    ...
    BuildMI(MBB, MBBI, DL, get(OR1K::MOVHI), *Reg).addImm(High);
    BuildMI(MBB, MBBI, DL, get(OR1K::ADDI), *Reg).addReg(*Reg).addImm(Low);
  } else {
