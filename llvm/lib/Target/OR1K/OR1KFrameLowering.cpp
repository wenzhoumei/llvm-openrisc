...
OR1KFrameLowering::OR1KFrameLowering(const OR1KSubtarget &STI)
...
void OR1KFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(OR1K::OR), FP)
        .addReg(SP)
        .addReg(SP)
  ...
  if (hasFP(MF)) {
    ...
    for (unsigned i = 0, e = CSI.size(); i < e; ++i) {
      ...
      if (I->getOpcode() == TargetOpcode::COPY && Info.isSpilledToReg()) {
        ...
        IsRestoreInst = (Info.getReg() == Reg.asMCReg()) &&
                        (LoadFI == FI);
      }
    ...
    BuildMI(MBB, I, DL, TII.get(OR1K::OR), SP).addReg(FP).addReg(FP);
  }
...
bool OR1KFrameLowering::restoreCalleeSavedRegisters(
  ...
  // Otherwise by default it restores with a series of OR1KInstrInfo::loadRegFromStackSlot
  return false;
}

// Eliminate ADJCALLSTACKDOWN, ADJCALLSTACKUP pseudo instructions
MachineBasicBlock::iterator OR1KFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (I->getOpcode() == OR1K::ADJCALLSTACKDOWN)
      Amount = -Amount;

    TII.adjustStackPtr(OR1K::R1, Amount, MBB, I);
  }

  return MBB.erase(I);
}

void OR1KFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                               BitVector &SavedRegs,
