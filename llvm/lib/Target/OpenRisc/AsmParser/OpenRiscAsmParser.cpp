//===- OpenRiscAsmParser.cpp - Parse OpenRisc assembly to MCInst instructions -===//
//
...
#define DEBUG_TYPE "openrisc-asm-parser"

struct OpenRiscOperand;

class OpenRiscAsmParser : public MCTargetAsmParser {

  SMLoc getLoc() const { return getParser().getTok().getLoc(); }

  OpenRiscTargetStreamer &getTargetStreamer() {
    MCTargetStreamer &TS = *getParser().getStreamer().getTargetStreamer();
    return static_cast<OpenRiscTargetStreamer &>(TS);
  }
...
#include "OpenRiscGenAsmMatcher.inc"
...
#undef GET_OPERAND_DIAGNOSTIC_TYPES
  ...
  OpenRiscAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
struct OpenRiscOperand : public MCParsedAsmOperand {
  ...
  OpenRiscOperand(KindTy K) : MCParsedAsmOperand(), Kind(K) {}

public:
  OpenRiscOperand(const OpenRiscOperand &o) : MCParsedAsmOperand() {
    Kind = o.Kind;
    StartLoc = o.StartLoc;
    EndLoc = o.EndLoc;
    switch (Kind) {
  ...
  static std::unique_ptr<OpenRiscOperand> createToken(StringRef Str, SMLoc S) {
    auto Op = std::make_unique<OpenRiscOperand>(Token);
    Op->Tok = Str;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
  ...
  static std::unique_ptr<OpenRiscOperand> createReg(unsigned RegNo, SMLoc S,
                                                  SMLoc E) {
    auto Op = std::make_unique<OpenRiscOperand>(Register);
    Op->Reg.RegNum = RegNo;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
  ...
  static std::unique_ptr<OpenRiscOperand> createImm(const MCExpr *Val, SMLoc S,
                                                  SMLoc E) {
    auto Op = std::make_unique<OpenRiscOperand>(Immediate);
    Op->Imm.Val = Val;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
...
unsigned OpenRiscAsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
                                                     unsigned Kind) {
...
bool OpenRiscAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
                                         MCStreamer &Out,
                                         const MCSubtargetInfo *STI) {
  Inst.setLoc(IDLoc);
...
bool OpenRiscAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((OpenRiscOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IDLoc;
    }
...
ParseStatus OpenRiscAsmParser::parsePCRelTarget(OperandVector &Operands) {
  ...
  Operands.push_back(OpenRiscOperand::createImm(Expr, S, getLexer().getLoc()));
  return ParseStatus::Success;
}

bool OpenRiscAsmParser::parseRegister(MCRegister &Reg, SMLoc &StartLoc,
                                    SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  Reg = OpenRisc::NoRegister;
...
ParseStatus OpenRiscAsmParser::parseRegister(OperandVector &Operands,
  ...
  if (HadParens)
    Operands.push_back(OpenRiscOperand::createToken("(", FirstS));
  SMLoc S = getLoc();
  SMLoc E = getParser().getTok().getEndLoc();
  getLexer().Lex();
  Operands.push_back(OpenRiscOperand::createReg(RegNo, S, E));

  if (HadParens) {
    getParser().Lex(); // Eat ')'
    Operands.push_back(OpenRiscOperand::createToken(")", getLoc()));
  }

  return ParseStatus::Success;
}

ParseStatus OpenRiscAsmParser::parseImmediate(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E;
  const MCExpr *Res;
  ...
  Operands.push_back(OpenRiscOperand::createImm(Res, S, E));
  return ParseStatus::Success;
}

ParseStatus OpenRiscAsmParser::parseOperandWithModifier(OperandVector &Operands) {
  return ParseStatus::Failure;
}
...
bool OpenRiscAsmParser::ParseInstructionWithSR(ParseInstructionInfo &Info,
  ...
  if ((Name.starts_with("wsr.") || Name.starts_with("rsr.") ||
    ...
    Operands.push_back(OpenRiscOperand::createToken(Name.take_front(3), NameLoc));
    ...
    // First operand is token for instruction
    Operands.push_back(OpenRiscOperand::createToken(Name, NameLoc));
...
bool OpenRiscAsmParser::ParseInstruction(ParseInstructionInfo &Info,
                                       StringRef Name, SMLoc NameLoc,
                                       OperandVector &Operands) {
  if (Name.starts_with("wsr") || Name.starts_with("rsr") ||
      Name.starts_with("xsr")) {
...
bool OpenRiscAsmParser::parseLiteralDirective(SMLoc L) {
  MCAsmParser &Parser = getParser();
  const MCExpr *Value;
...
ParseStatus OpenRiscAsmParser::parseDirective(AsmToken DirectiveID) {
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeOpenRiscAsmParser() {
  RegisterMCAsmParser<OpenRiscAsmParser> X(getTheOpenRiscTarget());
}
